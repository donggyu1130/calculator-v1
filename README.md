...
52.시나리오 추가
//50번 시나리오 참고해서 반대 경우의 시나리오도 추가한다
=>2를 누른다-2가 출력됨
'+'를 누른다-변화없음
3을 누른다-3이 출력됨
'-'를 누른다-5가 출력됨
2를 누른다-2가 출력됨
'='을 누른다-3이 출력됨
//시나리오 하나를 발견했으면 다른 시나리오도 예상해볼수있다
//나중 시나리오도 미리 예측해볼수있다:+,-가 아닌 다른 연산자가 추가된다면 이런 시나리오를 미리 고려해서 짜야겠다

53.시나리오 추가
//3수의 뻴셈 두자리 음수
=>-를 누른다-변화 없음
2를 누른다-2가 출력됨
2를 누른다-22가 출력됨 -를 누른다-'-'22가 출력됨
2를 누른다-2가 출력됨
2를 누른다-22가 출력됨 -를 누른다-'-'44가 출력됨
2를 누른다-2가 출력됨
2를 누른다-22가 출력됨
=을 누른다-'-'66이 출력됨
...
54.시나리오 검토
//시나리오는 사실 엉뚱할수록 좋다
//ex)말도 안되는 큰수를 써보기,말도 안되는 여러번 개산 해보기
...
1을 누른다-111111111111111이 출력됨
1을 누른다-1111111111111111이 출력됨
'='을 누른다-2222222222222222
//즉,1000조 단위에서 +연산이 가능하다

59.시나리오 추가🌟🌟//자리수가 틀려서 오류가 꽤 생김
//1000조단위 '-'계산
1을 누른다-1이 출력됨
1을 누른다-11이 출력됨
1을 누른다-111이 출력됨
...
'='을 누른다-'-'111111111111111이 출력됨
//1000조 단위의 '-'연산

60.숫자4 버튼 추가
숫자4 버튼이 존재한다
//최대한 작은 범위로 테스트를 시작한다
